...
export default function Inspector() {
  ...
  const { elements, matches, currentMatchId, selectedElementId, updateElement, updateDecorativeElement, deleteElement, deleteDecorativeElement } = store;
  ...
  if (!selectedEl) {
    return (
      <div className="w-80 border-l border-gray-200 bg-white p-6 flex flex-col shadow-sm z-50 overflow-y-auto">
        <div className="flex flex-col items-center justify-center text-gray-500 mb-8 py-4">
        ...
        <div className="pt-6 border-t border-gray-200">
          <h2 className="text-lg font-bold mb-4 text-gray-800 tracking-wider">
            전역 설정
          </h2>
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
            <span className="text-sm font-semibold text-gray-700">중복 인스턴스 허용</span>
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" className="sr-only peer" checked={store.overlapEnabled} onChange={(e) => store.setOverlapEnabled(e.target.checked)} />
              <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
          </div>
          <p className="text-xs text-gray-400 mt-2 ml-1">
            리드/래그 타임 적용 시, 동일한 버튼을 여러 번 누를 경우 인스턴스를 중복 생성합니다. (SportsCode 방식)
          </p>
        </div>
      </div>
    );
