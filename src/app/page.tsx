...
export default function Home() {
  ...
  const handleExport = () => {
    const activeMatch = matches.find(m => m.id === currentMatchId);
    exportToSportsCodeXML(getInstances(), getRows(), activeMatch?.name);
  };

  const handleExportMatch = (matchId: string) => {
    const match = matches.find(m => m.id === matchId);
    if (!match) return;
    exportToSportsCodeXML(match.instances, getRows(), match.name);
  };
  ...
  const handleCreateMatch = (e: React.FormEvent) => {
    e.preventDefault();
    const nameToUse = newMatchName.trim() || 'New Match';
    createNewMatch(nameToUse);
    setNewMatchName('');
             ...
             <div className="flex items-center gap-2 border-r border-gray-200 pr-4 mr-1">
              {timerStatus === 'idle' || timerStatus === 'stopped' ? (
                <button 
                  ...
                  onClick={() => {
                    if (timerStatus === 'stopped' || (!currentMatchId && matches.length === 0)) {
                      setShowMatchPrompt(true);
                    } else {
                      startRecording();
                    }
                ...
                <>
                  <button
                    className={`px-5 py-2 text-white rounded-lg text-lg font-bold flex items-center gap-3 shadow-md tracking-wider font-mono cursor-pointer transition-colors bg-red-600 hover:bg-red-700`}
                    onClick={() => {
